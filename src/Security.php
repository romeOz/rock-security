 ...
 *
 * - Encryption/decryption: {@see \rock\security\Security::encryptByKey()}, {@see \rock\security\Security::decryptByKey()},
 *   {@see \rock\security\Security::encryptByPassword()} and {@see \rock\security\Security::decryptByPassword()}
 * - Key derivation using standard algorithms: {@see \rock\security\Security::pbkdf2()} and {@see \rock\security\Security::hkdf()}
 * - Data tampering prevention: {@see \rock\security\Security::hashData()} and {@see \rock\security\Security::validateData()}
 * - Password validation: {@see \rock\security\Security::generatePasswordHash()} and {@see \rock\security\Security::validatePassword()}
 *
...
class Security implements ObjectInterface
{
     ...
     * which is very fast relative to {@see \rock\security\Security::encryptByPassword()}. The input key must be properly
     * random -- use {@see \rock\security\Security::generateRandomKey()} to generate keys.
     * The encrypted data includes a keyed message authentication code (MAC) so there is no need
     * to hash input or output data.
     * @param string $data the data to encrypt
     * @param string $inputKey the input to use for encryption and authentication
     * @param string $info optional context and application specific information, see {@see \rock\security\Security::hkdf()}
     * @return string the encrypted data
     * @see decryptByPassword()
     * @see encryptByKey()
     */
    ...
    /**
     * Verifies and decrypts data encrypted with {@see \rock\security\Security::encryptByPassword()}.
     ...
     * @throws SecurityException when HMAC generation fails.
     ...
     * the length of the hash algorithm output.
     * @return string the derived key
     * @throws SecurityException when hash generation fails due to invalid params given.
     */
    public function pbkdf2($algo, $password, $salt, $iterations, $length = 0)
    {
        if (function_exists('hash_pbkdf2')) {
            ...
            if ($outputKey === false) {
                throw new SecurityException('Invalid parameters to hash_pbkdf2()');
            }
            return $outputKey;
        }
     ...
     * generated by {@see \rock\security\Security::hashData()}.
     * @param string $key the secret key that was previously used to generate the hash for the data in {@see \rock\security\Security::hashData()}.
     * function to see the supported hashing algorithms on your system. This must be the same
     * as the value passed to {@see \rock\security\Security::hashData()} when generating the hash for the data.
     * @param boolean $rawHash this should take the same value as when you generate the data using {@see \rock\security\Security::hashData()}.
     ...
     * to {@see \rock\security\Security::validatePassword()}. For example,
     *
     * ```php
     * // generates the hash (usually done during user registration or when the password is changed)
     * $hash = Rock::$app->security->generatePasswordHash($password);
     * // ...save $hash in database...
     *
     * // during login, validate if the password entered is correct using $hash fetched from database
     * if (Rock::$app->security->validatePassword($password, $hash) {
     *     // password is good
     * } else {
     *     // password is bad
     * }
     ...
     * @throws SecurityException if the cost parameter is out of the range of 4 to 31.
     */
